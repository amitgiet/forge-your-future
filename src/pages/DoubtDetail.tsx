  ...
  return (
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className="sticky top-0 z-30 bg-background/95 backdrop-blur-sm border-b border-border">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-foreground truncate">Doubt Detail</h1>
          </div>
          {doubt.isResolved && (
            <span className="flex items-center gap-1 text-xs font-semibold text-primary bg-primary/10 px-2.5 py-1 rounded-full border border-primary/20">
      ...
      <div className="max-w-md mx-auto px-4 pt-4 space-y-4">
        {error && <div className="bg-destructive/10 border border-destructive/30 rounded-2xl p-3 text-sm text-destructive">{error}</div>}
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-semibold capitalize text-muted-foreground">{doubt.subject}</span>
            {doubt.chapterId && (
              <span className="text-[10px] text-muted-foreground truncate max-w-[120px]">· {doubt.chapterId}</span>
            )}
              ...
              .map((ans, i) => (
                <motion.div
                  ...
                  className={`bg-card border rounded-2xl p-4 mb-3 ${
                    ans.isAccepted ? 'border-primary/30 bg-primary/5' : 'border-border'
                  }`}
                >
                  ...
                  <div className="flex items-center gap-3 mt-3 pt-3 border-t border-border">
                    ...
                    {!doubt.isResolved && !ans.isAccepted && (
                      <button
                        ...
                        className="text-xs text-primary font-medium hover:underline transition-colors flex items-center gap-1"
                      >
      ...
      {/* Answer input bar */}
      <div className="fixed bottom-16 left-0 right-0 border-t border-border bg-background/95 backdrop-blur-sm px-4 py-3">
        <div className="max-w-md mx-auto flex gap-2 items-end">
          <textarea
            ...
            rows={2}
            className="flex-1 bg-card border border-border rounded-xl px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary resize-none"
          />
          <button
            onClick={handleSubmitAnswer}
            disabled={!answerBody.trim() || submitting}
            className="h-11 w-11 rounded-xl bg-primary flex items-center justify-center disabled:opacity-50 text-primary-foreground flex-shrink-0 shadow-sm"
          >
